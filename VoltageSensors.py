...
ws.append(["Timestamp", "rcVoltage (V)", "rcCurrent (A)", "rcPower (W)","","genVoltage (V)", "genCurrent (A)", "genPower (W)"])
wb.save(filepath)
...
try:
    for timestamp in range(600):    #roughly 10 min
        ...
        ws.append([timestamp,rcVoltage,rcCurrent,rcPower,"",genVoltage,genCurrent,genPower])
        wb.save(filepath) 
        time.sleep(0.25)
